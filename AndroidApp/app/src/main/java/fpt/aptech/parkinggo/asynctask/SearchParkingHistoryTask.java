...
public class SearchParkingHistoryTask extends AsyncTask<Void, Integer, ResponseEntity<?>> {
    private Activity activity;
    private CustomProgressDialog dialog;

    public SearchParkingHistoryTask(Activity activity){
    ...
    protected ResponseEntity<?> doInBackground(Void... voids) {
        String fromDate = ((TextView) activity.findViewById(R.id.a_parking_history_tv_fromdate)).getText().toString();
        String toDate = ((TextView) activity.findViewById(R.id.a_parking_history_tv_todate)).getText().toString();
        ...
        ResponseEntity<?> response = RestTemplateConfiguration
                .excuteRequest(uri+"all-parking-history/username/search?from-date="+fromDate+"&to-date="+toDate, HttpMethod.GET, request, ParkingHistoryRes[].class);
        return response;
    }
